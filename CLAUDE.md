...
`data/seen.json`. `funding.py` is not wired into the pipeline — callers invoke
`enrich_companies` explicitly, keeping the signals stage free of EDGAR's rate
limits.
...
- **`data/companies.csv` is hand-maintained** (`name,ats,token,headcount,source`)
  ...
  zero — never guess it. Note it *is* tracked in git, despite `README.md` saying
  otherwise.
- **`digest` commits `signals/<date>.md` when `company_signals` ran**, alongside
  `matches/<date>.md` and `data/seen.json`. `hygiene.DIGEST_PATH_PATTERN` must
  stay in sync with `paths.signals_path` — when it didn't, the 2026-08-17 run
  rendered and committed its digest, then had the push rejected as "not a digest
  commit" and failed. Nothing re-runs its way out of that: digest writes no
  checkpoint on failure, so every later run repeats the same rejection.
...
so the two can't drift: no files over 2MB, no generated output
(`data/runs/`, `logs/`, `data/raw/`), no conflict markers, branch names must
match `(feat|fix|chore|docs|test|refactor|ci)/slug`, and only digest commits may
land on `main` directly — everything else goes through a PR.
...
`SCOUT_EDGAR_DELAY`, `CLAUDE_BIN`, `NTFY_TOPIC`.
